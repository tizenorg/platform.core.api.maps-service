...
typedef int (*maps_plugin_is_service_supported_f) (maps_service_e service, bool *supported);
typedef int (*maps_plugin_is_data_supported_f) (maps_service_data_e data, bool *supported);
...
typedef int (*maps_plugin_geocode_inside_area_f) (const char *address,
						...
						void *user_data,
						int *request_id);
typedef int (*maps_plugin_geocode_by_structured_address_f) (const maps_address_h address,
						const maps_preference_h preference,
						maps_service_geocode_cb callback,
						void *user_data,
						int *request_id);
...
typedef int (*maps_plugin_search_route_f) (const maps_coordinates_h origin,
						...
						void *user_data, int *request_id);
typedef int (*maps_plugin_search_route_waypoints_f) (const maps_coordinates_h * waypoint_list,
						int waypoint_num,
						maps_preference_h preference,
						maps_service_search_route_cb callback,
						void *user_data,
...
/* Mapping */
typedef int (*maps_plugin_create_map_view_f) (const maps_view_h view, maps_plugin_map_view_ready_cb callback);
typedef int (*maps_plugin_destroy_map_view_f) (const maps_view_h view, maps_plugin_map_view_ready_cb callback);
typedef int (*maps_plugin_render_map_f) (const maps_view_h view, const maps_coordinates_h coordinates, const double zoom_factor,
						const double rotation_angle, maps_plugin_render_map_cb callback, void* user_data, int* request_id);
typedef int (*maps_plugin_move_center_f) (const maps_view_h view, const int delta_x, const int delta_y, maps_plugin_render_map_cb callback,
						void* user_data, int* request_id);
typedef int (*maps_plugin_set_scalebar_f) (const maps_view_h view, bool enable);
typedef int (*maps_plugin_get_scalebar_f) (const maps_view_h view, bool *enabled);
typedef int (*maps_plugin_draw_map_f) (const maps_view_h view, Evas* canvas, const int x, const int y, const int width, const int height);
typedef int (*maps_plugin_on_object_f) (const maps_view_h view, const maps_view_object_h object, const maps_view_object_operation_e operation);
typedef int (*maps_plugin_screen_to_geography_f) (const maps_view_h view, const int x, const int y, maps_coordinates_h* coordinates);
typedef int (*maps_plugin_geography_to_screen_f) (const maps_view_h view, const maps_coordinates_h coordinates, int* x, int* y);
typedef int (*maps_plugin_get_min_zoom_level_f) (const maps_view_h view, int *min_zoom_level);
typedef int (*maps_plugin_get_max_zoom_level_f) (const maps_view_h view, int *max_zoom_level);
typedef int (*maps_plugin_get_center_f) (const maps_view_h view, maps_coordinates_h *coordinates);
...
typedef struct _plugin_s {
	...
	GThread *thread;	/* Request queue thread: there is
				   happening delivery of request from app to plugin */

	/* Flag: is the plugin in use */
	volatile bool is_working;	/* It is discouraged to use this approach */
	/* probably, it must be exchanged with event dispite of  performance trade-off */

	/*maps_int_hashtable_h capabilities; */ /* The table of plugin capabilities */

	GMutex pending_request_mutex;	/* Mutex for synchronizing the
					   map of pending requests */
	GHashTable *pending_request_maps;	/* Map of pending requests */
} plugin_s;

/* TODO: Refactor to encapsulate everything in a single calss
*  plugin::module */
