...
map_view_object_h view::polyline_constructor::construct(map_view_h view,
							...
							const int width)
{
	...
	map_view_object_h polyline = NULL;
	do {
		...
		__error = map_view_object_poly_set_points(polyline,
							  coordinates);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* 3. Set the polyline color */
		__error = map_view_object_polyline_set_color(polyline,
							     r, g, b, a);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* 4. Set the polyline width */
		__error = map_view_object_polyline_set_width(polyline, width);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* 5. Set the polyline visible */
		__error = map_view_object_set_visible(polyline, true);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* Notify view, that the object was constructed */
		_map_view_object_set_view(polyline, view);
		if(view)
