...
void view::gesture_detector_script::on_long_press_timer()
{
	...
	/*if(_info._cur_gesture != MAP_GESTURE_PRESS)*/
	...
	_info._cur_gesture = MAP_GESTURE_PRESS;
...
bool view::gesture_detector_script::test_long_press_gesture()
{
	...
	if (!is_gesture_available(MAP_GESTURE_PRESS))
		return false;
...
bool view::gesture_detector_script::test_panning_gesture_finished()
{
	...
	if(_info._cur_gesture == MAP_GESTURE_PAN) {
...
{
	/* Check if the gesture is available */
	if (!is_gesture_available(MAP_GESTURE_PAN))
		return false;
	...
	_info._cur_gesture = MAP_GESTURE_PAN;
	MAPS_LOGI("GESTURE PAN DETECTED");
	_gp.on_pan(0);
	return true;
}

bool view::gesture_detector_script::test_tilt_gesture()
{
	/* TODO: Add MAP_GESTURE_TILT to map_gesture_e enum */
	/* Check if the gesture is available */
	/*if (!is_gesture_available(MAP_GESTURE_TILT))
		return false;*/
	...
	/*_info._cur_gesture = MAP_GESTURE_TILT;*/
	/*MAPS_LOGI("GESTURE TILT DETECTED");*/
