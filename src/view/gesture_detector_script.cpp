...
void view::gesture_detector_script::on_long_press_timer()
{
	...
	_info._cur_gesture = MAP_CONTROL_LONG_PRESS;
...
bool view::gesture_detector_script::test_long_press_gesture()
{
	...
	if (!is_gesture_available(MAP_CONTROL_LONG_PRESS))
		return false;
...
{
	/* Check if the gesture is available */
	if (!is_gesture_available(MAP_CONTROL_DOUBLE_TAP))
		return false;
	...
	if((time > 0) && (time < __CLICK_DURATION)) {
		_info._cur_gesture = MAP_CONTROL_DOUBLE_TAP;
...
bool view::gesture_detector_script::test_tap_gesture()
{
	...
	/* Check if the gesture is available */
	if (!is_gesture_available(MAP_CONTROL_TAP))
		return false;
	...
	if((time > 0) && (time < __CLICK_DURATION)) {
		...
		if(get_trajectory_effective_length(p1, p2) < __CLICK_AREA) {
			_info._cur_gesture = MAP_CONTROL_TAP;
...
bool view::gesture_detector_script::test_flick_gesture()
{
	...
	/* Check if the gesture is available */
	if (!is_gesture_available(MAP_CONTROL_FLICK))
		return false;
...
bool view::gesture_detector_script::test_zoom_rotate_gesture()
{
	...
	if (!is_gesture_available(MAP_GESTURE_PINCH_ZOOM))
#else
	/* Check if the gesture is available */
	if ((!is_gesture_available(MAP_GESTURE_ROTATE))
	...
	if(_info._cur_gesture != MAP_GESTURE_PINCH_ZOOM)
		if(!finger_dragged_enough(0) && finger_dragged_enough(1))
			return false;

	/* No special checks are need here */
	_info._cur_gesture = MAP_GESTURE_ROTATE;
