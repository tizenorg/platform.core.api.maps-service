...
extern int _map_view_object_route_set_content(map_view_object_h route,
...
map_view_object_h view::route_constructor::construct(map_view_h view,
						     maps_route_h route)
{
	if (!route) {
	...
	map_view_object_h route_object = NULL;
	do {
		...
		/* 2. Assign route object to route layer */
		__error =  _map_view_object_route_set_content(route_object,
							      route);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* 3. Set Route visible */
		__error = map_view_object_set_visible(route_object, true);
		if (__error != MAPS_ERROR_NONE)
			break;

		/* Notify view, that the object was constructed */
		_map_view_object_set_view(route_object, view);
		if(view)
...
extern int _map_view_object_create(const map_view_object_type_e type,
				   map_view_object_h *object);

extern int _map_view_object_destroy(map_view_object_h object);

extern int _map_view_object_layer_set_route(map_view_object_h layer,
...
int view::route_constructor::add_marker(map_view_object_h route_object,
					 ...
					 map_view_marker_type_e type)
{
	...
	map_view_object_h marker = mc.construct(coords, image_file, type);
	if(!marker)
		return mc.get_error();
	return map_view_object_layer_add_object(route_object, marker);
}
						     ...
						     void *user_data)
{
	if(!data || !user_data)
		return false;
	add_marker(user_data, data, MAP_VIEW_MARKER_NONE);
	return true;
}

int view::route_constructor::add_trajectory(map_view_object_h route_object,
					    maps_item_list_h points)
{
	...
	map_view_object_h polyline = pc.construct(points, 255, 0, 0, 0, 1);
	if(!polyline)
		return pc.get_error();
	int e = map_view_object_layer_add_object(route_object, polyline);
...
void view::route_constructor::add_start_marker(map_view_object_h route_object,
					       maps_route_h route)
{
	...
	if (__error == MAPS_ERROR_NONE) {
		__error = add_marker(route_object,
				     origin,
				     MAP_VIEW_MARKER_START);
		maps_coordinates_destroy(origin);
	}
}

void view::route_constructor::add_finish_marker(map_view_object_h route_object,
						maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
	...
	if (__error == MAPS_ERROR_NONE) {
		__error = add_marker(route_object,
				     destination,
				     MAP_VIEW_MARKER_FINISH);
		maps_coordinates_destroy(destination);
	}
}

void view::route_constructor::add_route_path(map_view_object_h route_object,
					     maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
...
void view::route_constructor::add_route_segments(map_view_object_h route_object,
						 maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
					       ...
					       void *user_data)
{
	...
	map_view_object_h route_object = (map_view_object_h)user_data;
	if(!route_object) {
						     ...
						     maps_route_h route)
{
	...
	do {
		/* 1. Create a visual object for route */
		__error = _map_view_object_create(map_view_h view,
		...
		/* 3. Assign route object to route layer */
		__error =  _map_view_object_layer_set_route(route_object,
							    route);
		if (__error != MAPS_ERROR_NONE)
			break;
