...
void view::gesture_processor::on_long_press()
{
	...
	q()->push(construct_gesture_command(MAP_GESTURE_PRESS, c,
					    false, .0, false, .0, .0));
	...
	if(ed) {
		_map_event_data_set_gesture_type(ed, MAP_GESTURE_PRESS);
...
void view::gesture_processor::on_pan(int finger_no)
{
	...
	* the geographical */
	...
	q()->push(construct_gesture_command(MAP_GESTURE_PAN, c,
					    false, .0, false, .0, .0));
	maps_coordinates_destroy(c);
#endif /* _MOVE_CENTER_COMMAND_DEFINED_ */
	...
	map_event_data_h ed =
		_map_view_create_event_data(MAP_EVENT_GESTURE);
	if(ed) {
		_map_event_data_set_gesture_type(ed, MAP_GESTURE_PAN);
