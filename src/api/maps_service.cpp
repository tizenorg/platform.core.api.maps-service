...
EXPORT_API int maps_service_provider_is_widget_supported(const maps_service_h maps,
	maps_service_widget_e data, bool *supported)
{
	if (!maps || !supported)
		return MAPS_ERROR_INVALID_PARAMETER;
	if ((data < MAPS_WIDGET_TRAFFIC) || (data > MAPS_WIDGET_SCALEBAR))
		return MAPS_ERROR_INVALID_PARAMETER;

	const plugin::plugin_s *p = __extract_plugin(maps);
	if (!p)
		return MAPS_ERROR_NOT_SUPPORTED;
	return p->interface.maps_plugin_is_widget_supported(data, supported);
}
