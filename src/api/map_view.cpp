...
extern int _map_event_data_set_tilt(map_event_data_h event,
				    const double tilt);
...
static void __map_view_parent_resize_cb(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	map_view_s *v = (map_view_s*)data;

	int w, h;
	evas_object_geometry_get(v->parent, NULL, NULL, &w, &h);
	map_view_resize(v, w, h);
}

static void __map_view_parent_move_cb(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	map_view_s *v = (map_view_s*)data;

	int x, y;
	evas_object_geometry_get(v->parent, &x, &y, NULL, NULL);
				...
				map_view_h *view)
{
	...
	*/
	...
	evas_object_event_callback_add(v->parent, EVAS_CALLBACK_MOVE, __map_view_parent_move_cb, v);
	evas_object_event_callback_add(v->parent, EVAS_CALLBACK_RESIZE, __map_view_parent_resize_cb, v);
	...
	v->gesture_actions[MAP_GESTURE_PRESS] = MAP_ACTION_NONE;
	...
	*view = (map_view_h) v;
	...
	__get_plugin_interface(v)->maps_plugin_get_max_zoom_level(
								  &v->max_zoom_level);
	__get_plugin_interface(v)->maps_plugin_get_min_tilt(&v->min_tilt);
	__get_plugin_interface(v)->maps_plugin_get_max_tilt(&v->max_tilt);
				   ...
				   const maps_coordinates_h coordinates)
{
	...
	if(ed) {
		_map_event_data_set_action_type(ed, MAP_ACTION_SET_CENTER);
...
int _map_view_move_center(map_view_h view, const int delta_x, const int delta_y)
{
	...
	*  similarly as it is done in maps service */
	...
	map_event_data_h ed =
		_map_view_create_event_data(MAP_EVENT_ACTION);
	if(ed) {
		_map_event_data_set_action_type(ed,
...
EXPORT_API int map_view_set_tilt(map_view_h view, const double tilt)
{
	...
	map_event_data_h ed =
		_map_view_create_event_data(MAP_EVENT_ACTION);
	if(ed) {
		_map_event_data_set_action_type(ed, MAP_ACTION_TILT);
		_map_event_data_set_tilt(ed, v->tilt);
		_map_view_invoke_event_callback(v, ed);
	}
...
EXPORT_API int map_view_get_mode(map_view_h view, map_view_mode_e* mode)
{
	MAPS_LOG_API;
	if (!view || !mode)
		return MAPS_ERROR_INVALID_PARAMETER;
...
static bool __map_object_poly_collect_points_cb(int index, int total,
