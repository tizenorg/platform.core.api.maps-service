 ...
 */
typedef struct _map_view_event_callback_s {
	map_view_on_event_cb user_cb;
	void *user_data;
} map_view_event_callback_s;
...
/*
 * The structure of map ready callback info
 */
typedef struct _map_view_map_ready_callback_s {
	map_view_on_map_ready_cb user_cb;
	void *user_data;
} map_view_ready_callback_s;
...
typedef struct _map_view_s {
	...
	bool control_available[MAP_CONTROL_NONE + 1];

	/* Continuous gesture info */
	view::finger_event_stream *finger_stream;

	/* Map View callbacks */
	map_view_event_callback_s event_callbacks[MAP_EVENT_READY + 1];
	map_view_ready_callback_s ready_callback;
...
void *_map_view_get_maps_service_instance(map_view_h view)
{
	if(!view)
		return NULL;
	map_view_s *v = (map_view_s *)view;
				 ...
				 void *user_data)
{
	...
	*/
	...
	if (v->ready_callback.user_cb) {
		v->ready_callback.user_cb(MAPS_ERROR_NONE, v, v->ready_callback.user_data);
	}
...
static Eina_Bool __map_view_animator_cb(void *data)
{
	...
	int width = 0;
	int height = 0;
	map_view_get_screen_location(v, NULL, NULL, &width, &height);

	/* The Plugin should draw a map. */
	__get_plugin_interface(v)->maps_plugin_draw_map(v->canvas, 0, 0, width, height);
...
#endif
	map_view_set_center(v, v->center);
}
...
int _map_view_get_inertia_enabled(map_view_h view, bool *enabled)
{
	MAPS_LOG_API;
	if (!view || !enabled)
		return MAPS_ERROR_INVALID_PARAMETER;
				...
				map_view_h *view)
{
	...
	const int event_callback_cnt = sizeof(v->event_callbacks) / sizeof(v->event_callbacks[0]);
	memset(v->event_callbacks, 0x00, event_callback_cnt);
	...
	*/
	...
	v->gesture_available[MAP_GESTURE_LONG_PRESS] = false;
	v->gesture_available[MAP_GESTURE_NONE] = false;
#endif


	/* Assign gestures to actions */
	memset(v->gesture_actions, 0x00, sizeof(v->gesture_actions) / sizeof(v->gesture_actions[0]));
	v->gesture_actions[MAP_VIEW_EVENT_SCROLL] = MAP_ACTION_SCROLL;
	v->gesture_actions[MAP_VIEW_EVENT_DOUBLE_TAP_ZOOM] = MAP_ACTION_ZOOM_IN;
	/* TODO: have to separate one-finger double tap from two-finger double tap internally.
	    one-finger double tap is zoom in and two-finger tap is zoom out.
	v->gesture_actions[MAP_VIEW_EVENT_TWO_FINGER_TAP] = MAP_ACTION_ZOOM_OUT;
	*/
	v->gesture_actions[MAP_VIEW_EVENT_PINCH_ZOOM] = MAP_ACTION_ZOOM;
	v->gesture_actions[MAP_VIEW_EVENT_ROTATE] = MAP_ACTION_ROTATE;
	v->gesture_actions[MAP_VIEW_EVENT_TILT] = MAP_ACTION_NONE;

	/* Set up gesture availability */
	v->gesture_available[MAP_GESTURE_SCROLL] = true;	
	v->gesture_available[MAP_GESTURE_DOUBLE_TAP_ZOOM] = true; 
	v->gesture_available[MAP_GESTURE_PINCH_ZOOM] = true;
	v->gesture_available[MAP_GESTURE_ROTATE] = false;
	v->gesture_available[MAP_GESTURE_TILT] = false;
	v->gesture_available[MAP_GESTURE_NONE] = false;
	...
	*view = (map_view_h) v;
	...
	_map_view_set_inertia_enabled(v, true);
			      ...
			      const double angle)
{
	...
	if (zoom_changed) {
		...
		if(ed) {
			...
			_map_event_data_set_scale_factor(ed, v->zoom_factor);
...
int map_view_set_scale_factor(const map_view_h view,
					const double factor)
{
	MAPS_LOG_API;
	if (!view)
...
int map_view_get_scale_factor(const map_view_h view, double *factor)
{
	MAPS_LOG_API;
	if (!view || !factor)
		return MAPS_ERROR_INVALID_PARAMETER;
				   ...
				   const maps_coordinates_h coordinates)
{
	...
	int error = map_view_get_scale_factor(view, &zoom_factor);
				  ...
				  maps_coordinates_h *coordinates)
{
	...
	return __get_plugin_interface(view)->maps_plugin_screen_to_geography(
		x - posx, y - posy, coordinates);
}
				  ...
				  int *x, int *y)
{
	...
	int posy = 0;
	map_view_get_screen_location(view, &posx, &posy, NULL, NULL);
	int error = __get_plugin_interface(view)->maps_plugin_geography_to_screen(
		coordinates, x, y);
	*x += posx;
	*y += posy;
	return error;
...
{
	MAPS_LOG_API;
	if (!view) return MAPS_ERROR_INVALID_PARAMETER;
	if ((type < MAP_TYPE_NORMAL) || (type > MAP_TYPE_TERRAIN))
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_s *v = (map_view_s *) view;
	int ret = MAPS_ERROR_NONE;

	ret = __get_plugin_interface(view)->maps_plugin_set_map_type(type);
	if (ret != MAPS_ERROR_NONE) {
		MAPS_LOGE("Couldn't set map type");
	} else {
		v->map_type = type;
	}
	return ret;
...
EXPORT_API int map_view_set_buildings_enabled(map_view_h view, const bool enabled)
{
	...
	ret = __get_plugin_interface(view)->maps_plugin_set_buildings_enabled(enabled);
	if (ret != MAPS_ERROR_NONE) {
		MAPS_LOGE("Couldn't set building enabled");
	} else {
		v->buildings_enabled = true;
	}
	
	return ret;
}

EXPORT_API int map_view_get_buildings_enabled(map_view_h view, bool *enabled)
{
	MAPS_LOG_API;
	if (!view) return MAPS_ERROR_INVALID_PARAMETER;
	map_view_s *v = (map_view_s *) view;
...
EXPORT_API int map_view_set_traffic_enabled(map_view_h view, const bool enabled)
{
	...
	ret = __get_plugin_interface(view)->maps_plugin_set_traffic_enabled(enabled);
	if (ret != MAPS_ERROR_NONE) {
		MAPS_LOGE("Couldn't set building enabled");
	} else {
		v->traffic_enabled = true;
	}
	return ret;
}

EXPORT_API int map_view_get_traffic_enabled(map_view_h view, bool *enabled)
{
	if (!view) return MAPS_ERROR_INVALID_PARAMETER;
	map_view_s *v = (map_view_s *) view;
...
EXPORT_API int map_view_set_public_transit_enabled(map_view_h view, const bool enabled)
{
	...
	ret = __get_plugin_interface(view)->maps_plugin_set_public_transit_enabled(enabled);
	if (ret != MAPS_ERROR_NONE) {
		MAPS_LOGE("Couldn't set building enabled");
	} else {
		v->public_transit_enabled = true;
	}
	return ret;
}

EXPORT_API int map_view_get_public_transit_enabled(map_view_h view, bool *enabled)
{
	MAPS_LOG_API;
	if (!view) return MAPS_ERROR_INVALID_PARAMETER;
	map_view_s *v = (map_view_s *) view;
...
EXPORT_API int map_view_set_language(map_view_h view, const char *language)
{
	...
	ret = __get_plugin_interface(view)->maps_plugin_set_language(language);
	if (ret != MAPS_ERROR_NONE) {
		MAPS_LOGE("Couldn't set display language of map view");
	} else {
		g_free(v->language);
		maps_set_string(language, _MAPS_VIEW_LANGUAGE_MAX_LENGTH, &v->language);
	}
	return ret;
}
...
EXPORT_API int map_view_set_screen_location(const map_view_h view,
			  const int x, const int y,
...
EXPORT_API int map_view_get_screen_location(const map_view_h view,
			   int *x, int *y,
...
EXPORT_API int map_view_set_visibility(const map_view_h view, const bool visible)
{
	MAPS_LOG_API;
	if (!view)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_view_get_visibility(const map_view_h view, bool *visible)
{
	MAPS_LOG_API;
	if (!view || !visible)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_view_set_map_ready_cb(map_view_h view, 
				  map_view_on_map_ready_cb user_cb,
				  void *user_data)
{
	MAPS_LOG_API;
	...
	map_view_s *v = (map_view_s *) view;
	v->ready_callback.user_cb = user_cb;
	v->ready_callback.user_data = user_data;

	return MAPS_ERROR_NONE;
}

EXPORT_API int map_view_unset_map_ready_cb(map_view_h view)
{
	MAPS_LOG_API;
	if (!view) return MAPS_ERROR_INVALID_PARAMETER;

	map_view_s *v = (map_view_s *) view;
	v->ready_callback.user_cb = NULL;
	v->ready_callback.user_data = NULL;

	return MAPS_ERROR_NONE;
}

EXPORT_API int map_view_set_event_cb(map_view_h view,
					   const map_view_event_e event,
					   map_view_on_event_cb user_cb,
					   void *user_data)
{
	MAPS_LOG_API;
	if (!view || !user_cb)
		return MAPS_ERROR_INVALID_PARAMETER;
	if((event <= MAP_VIEW_EVENT_NONE) || (event >= MAP_VIEW_EVENT_NONE))
		return MAPS_ERROR_INVALID_PARAMETER;

	map_view_s *v = (map_view_s *) view;
	v->event_callbacks[event].user_cb = user_cb;
	v->event_callbacks[event].user_data = user_data;

	return MAPS_ERROR_NONE;
}

EXPORT_API int map_view_unset_event_cb(map_view_h view,
					     const map_view_event_e event)
{
	MAPS_LOG_API;
	if (!view)
	...
	map_view_s *v = (map_view_s *) view;
	v->event_callbacks[event].user_cb = NULL;
	v->event_callbacks[event].user_data = NULL;
				     ...
				     map_event_data_h event_data)
{
	...
	v->event_callbacks[type].user_cb(MAPS_ERROR_NONE, type, event_data,
					  v->event_callbacks[type].user_data);
}

#if 0
void _map_view_invoke_ready_callback(map_view_h view,
				     map_event_data_h event_data)
{
	if(!view || !event_data)
		return;
	...
	map_event_type_e type = MAP_EVENT_READY;
	map_event_data_get_type(event_data, &type);

	v->ready_callback.user_cb(MAPS_ERROR_NONE, MAP_EVENT_READY, event_data, v->ready_callback.user_data);
}
#endif
