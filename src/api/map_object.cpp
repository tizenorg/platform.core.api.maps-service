...
static int __map_view_marker_data_create(void **marker)
{
	...
	do {
		...
		m->width = 0;
		m->height = 0;
		m->z_order = MAP_OBJECT_Z_ORDER_BASE;
...
{
	if (!marker || (z_order > MAP_OBJECT_Z_ORDER_UP || z_order < MAP_OBJECT_Z_ORDER_DOWN))
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
	if (!m)
		return MAPS_ERROR_INVALID_PARAMETER;
	...
	/* Get new z-index */
	int new_z_order = m->z_order;
	if (z_order == MAP_OBJECT_Z_ORDER_UP) 
		new_z_order++;
	else if (z_order == MAP_OBJECT_Z_ORDER_DOWN)
		new_z_order--;
	else
		new_z_order = z_order;

	if (new_z_order < MAP_OBJECT_Z_ORDER_BOTTOM)
		new_z_order = MAP_OBJECT_Z_ORDER_BOTTOM;
	else if (new_z_order > MAP_OBJECT_Z_ORDER_TOP)
		new_z_order = MAP_OBJECT_Z_ORDER_TOP;

	/* Notify view, that the object specific preferences is changed */
	if (m->z_order != new_z_order)
	{
		m->z_order = new_z_order;
