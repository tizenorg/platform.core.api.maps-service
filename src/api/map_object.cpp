...
static bool __map_object_group_object_set_view_cb(int index, int total, map_object_h object, void *user_data)
{
...
int _map_object_set_view(map_object_h object, map_view_h view)
{
	...
	if (type == MAP_OBJECT_GROUP)
		map_object_group_foreach_object(object, __map_object_group_object_set_view_cb, view);
#endif
					  ...
					  const maps_coordinates_list_h points)
{
	...
	maps_coordinates_list_destroy(p->points);
	//const int error = maps_coordinates_list_clone(points, &p->points);
	const int error = MAPS_ERROR_INVALID_OPERATION;
	MAPS_LOGE("ERROR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
