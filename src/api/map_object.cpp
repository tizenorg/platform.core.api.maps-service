...
extern int  _map_view_on_object_operation(map_view_h view, map_object_h object,	map_object_operation_e operation);

static map_view_h __get_view(const map_object_h object);
static map_view_polyline_data_s *__get_polyline_data(const map_object_h object);
static map_view_polygon_data_s *__get_polygon_data(const map_object_h object);
...
int _map_object_create(map_object_type_e type, map_object_h *object)
{
	if (!object)
		return MAPS_ERROR_INVALID_PARAMETER;
	if ((type < MAP_OBJECT_POLYLINE) || (type > MAP_OBJECT_MARKER))
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_create_marker(maps_coordinates_h coordinates,
	const char *image_file_path, map_marker_type_e type, map_object_h *marker)
{
	if (!coordinates || !marker)
		return MAPS_ERROR_INVALID_PARAMETER;
	if((type < MAP_MARKER_PIN) || (type > MAP_MARKER_STICKER))
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_create_polyline(maps_coordinates_list_h coordinates,
...
EXPORT_API int map_object_create_polygon(maps_coordinates_list_h coordinates,
	unsigned char r, unsigned char g, unsigned char b, unsigned char a, map_object_h *polygon)
{
...
EXPORT_API int map_object_get_type(map_object_h object, map_object_type_e *type)
{
	if (!object || !type)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_move(map_object_h object, maps_coordinates_h coordinates)
{
...
EXPORT_API int map_object_set_visible(map_object_h object, bool visible)
{
	if (!object)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_object_s *o = (map_object_s *)object;
...
EXPORT_API int map_object_polyline_set_polyline(map_object_h polyline, maps_coordinates_list_h points)
{
	if (!polyline || !points)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_polyline_set_width(map_object_h polyline, int width)
{
	if (!polyline)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_polyline_get_width(const map_object_h polyline, int *width)
{
	if (!polyline || !width)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_polygon_set_polygon(map_object_h polygon, maps_coordinates_list_h points)
{
	if (!polygon || !points)
		return MAPS_ERROR_INVALID_PARAMETER;
...
EXPORT_API int map_object_marker_resize(map_object_h marker, int width, int height)
{
	if (!marker)
		return MAPS_ERROR_INVALID_PARAMETER;
	if (width < 0 || height < 0)
...
EXPORT_API int map_object_marker_set_image_file(map_object_h marker, const char *file_path)
{
	if (!marker || !file_path)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
	if (!m)
		return MAPS_ERROR_INVALID_PARAMETER;
	const int error = maps_set_string(file_path, _MAP_MARKER_FILE_PATH_MAX_LENGTH, &m->file_path);
...
EXPORT_API int map_object_marker_get_image_file(const map_object_h marker, char **file_path)
{
	if (!marker || !file_path)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
	if (!m)
		return MAPS_ERROR_INVALID_PARAMETER;
	return maps_get_string(m->file_path, _MAP_MARKER_FILE_PATH_MAX_LENGTH, file_path);
}

EXPORT_API int map_object_marker_get_coordinates(const map_object_h marker, maps_coordinates_h *coordinates)
{
	if (!marker || !coordinates)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
...
EXPORT_API int map_object_marker_set_size(map_object_h marker, int width, int height)
{
	if (!marker)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
...
int _map_object_marker_set_type(map_object_h marker, map_marker_type_e type)
{
	if (!marker)
		return MAPS_ERROR_INVALID_PARAMETER;
	if ((type < MAP_MARKER_PIN) || (type > MAP_MARKER_STICKER))
...
EXPORT_API int map_object_marker_get_type(const map_object_h marker, map_marker_type_e *type)
{
	if (!marker || !type)
		return MAPS_ERROR_INVALID_PARAMETER;
	map_view_marker_data_s *m = __get_marker_data(marker);
...
EXPORT_API int map_object_marker_set_z_order(map_object_h marker, int z_order)
{
