											...
											const char *path)
{
	...
	image_util_colorspace_e cs = IMAGE_UTIL_COLORSPACE_RGBA8888;
	char *fname = NULL;
	...
	do {
		...
		if (cs != IMAGE_UTIL_COLORSPACE_RGBA8888 && cs != IMAGE_UTIL_COLORSPACE_BGRA8888) {
		...
		if (type == MAPS_VIEW_SNAPSHOT_JPEG) {
			error = image_util_encode_jpeg(image_buffer, w, h, cs, quality, fname);
			...
			if (cs == IMAGE_UTIL_COLORSPACE_RGBA8888) {
				MAPS_LOGD("RGBA -> BGRA");
				__convert_rgba_to_bgra(image_buffer, w, h);
				cs = IMAGE_UTIL_COLORSPACE_BGRA8888;
			}
