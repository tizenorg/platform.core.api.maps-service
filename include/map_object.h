...
/**
 * @brief	Enumerations of z-order types
 * @since_tizen 3.0
 */
enum {
	MAP_OBJECT_Z_ORDER_BASE,

	MAP_OBJECT_Z_ORDER_TOP = 100,

	MAP_OBJECT_Z_ORDER_BOTTOM = -100,

	MAP_OBJECT_Z_ORDER_UP = 101,

	MAP_OBJECT_Z_ORDER_DOWN = -101,
};
 ...
 */
int map_object_move(const map_object_h object,
 ...
 */
int map_object_set_visible(const map_object_h object, const bool visible);
 ...
 * @see map_object_polyline_foreach_point()
 */
int map_object_polyline_set_polyline(const map_object_h polyline,
 ...
 */
int map_object_polyline_foreach_point(const map_object_h polyline,
 ...
 * @see map_object_create_polyline()
 */
int map_object_polyline_set_color(const map_object_h polyline,
				  const unsigned char r,
				  const unsigned char g,
				  const unsigned char b,
				  const unsigned char a);
 ...
 * @see map_object_create_polyline()
 */
int map_object_polyline_set_width(const map_object_h polyline, const int width);
 ...
 * @see map_object_polygon_foreach_point()
 */
int map_object_polygon_set_polygon(const map_object_h polygon,
				   const maps_coordinates_list_h points);
 ...
 */
int map_object_polygon_foreach_point(const map_object_h polygon,
				     maps_coordinates_cb callback,
				     void *user_data);
 ...
 * @see map_object_create_polygon()
 */
int map_object_polygon_set_fill_color(const map_object_h polygon,
				      const unsigned char r,
				      const unsigned char g,
				      const unsigned char b,
				      const unsigned char a);
 ...
 * @see maps_coordinates_create()
 */
int map_object_marker_set_coordinates(const map_object_h marker,
				      const maps_coordinates_h coordinates);
 ...
 *
 * @param[in]	marker	The marker object handle
 * @param[in]	width	The marker width on the screen
 * @param[in]	height	The marker height on the screen
 * @return	0, otherwise a negative error value
 * @retval	#MAPS_ERROR_NONE Successful
 * @retval	#MAPS_ERROR_INVALID_PARAMETER Invalid parameter
 *
 ...
 */
int map_object_marker_resize(const map_object_h marker,
 ...
 * @see map_object_create_marker()
 */
int map_object_marker_set_image_file(const map_object_h marker,
 ...
 * \n The @a z_order must be in range of [-100, 100], or some macros.

 * and it can be also relative value like MAP_OBJECT_Z_ORDER_UP or MAP_OBJECT_Z_ORDER_DOWN.
 * These are macros to change z-order relatively.
 * @since_tizen 3.0
 *
 * @param[in]	marker		The marker object handle
 ...
 * | z_order                         | means                                |
 * | :------------------------------ | :----------------------------------- |
 * | -100 ~ 100                      | available range                      |
 * | MAP_OBJECT_Z_ORDER_BASE         | default value, equals to 0           |
 * | MAP_OBJECT_Z_ORDER_UP           | increase z-order                     |
 * | MAP_OBJECT_Z_ORDER_DOWN         | decrease z-order                     |
 * | MAP_OBJECT_Z_ORDER_TOP          | the top z-order, equals to 100       |
 * | MAP_OBJECT_Z_ORDER_BOTTOM       | the bottom z-order, equals to -100   |
 * @return	0, otherwise a negative error value
 * @retval	#MAPS_ERROR_NONE Successful
 * @retval	#MAPS_ERROR_INVALID_PARAMETER Invalid parameter
 *
 ...
 */
int map_object_marker_set_z_order(const map_object_h marker, const int z_order);
