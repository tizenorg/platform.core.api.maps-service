...
typedef enum _map_marker_type_e {
	...
	/** Indicates the POI (Point of Interest)  marker type (temp)*/
	MAP_MARKER_POI = MAP_MARKER_PIN,
			  ...
			  void *user_data)
{
	...
	error = maps_coordinates_get_latitude_longitude(point, &latitude, &longitude);
	if(error != MAPS_ERROR_NONE) {
		// Handle the error
	}
