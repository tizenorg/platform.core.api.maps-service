 ...
 *  * Gesture: the user performed a gesture over the map; examples of gestures
 *  are tap, double tap, rotate map, long press, etc. Gestures are listed in
 *  #map_gesture_e
 ...
 *
 * @par Enumerations available by Event Type
 ...
 * @details The handle of Maps View Event Data instance which is retrieved in
...
typedef enum _map_gesture_e {
	...
	/** Indicates the zoom user gesture (performing with pinch) */
	MAP_GESTURE_ZOOM,

	/** Indicates the single finger zoom user gesture
	* (the gesture consist of initial click and following pan) */
 ...
 * @details This enumeration is valuable if the event type is #MAP_EVENT_ACTION
 * @since_tizen 3.0
 */
...
typedef enum _map_event_type_e {
	...
	/** Indicates the user's gesture event callback */
	MAP_EVENT_GESTURE,

	/** Indicates the map view action callback */
	MAP_EVENT_ACTION,
