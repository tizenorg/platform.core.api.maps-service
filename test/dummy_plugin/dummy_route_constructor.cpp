...
int view::dummy_route_constructor::add_marker(map_view_object_h route_object,
					 ...
					 map_view_marker_type_e type)
{
	...
	map_view_object_h marker = mc.construct(NULL, coords, image_file, type);
	if(!marker)
		return mc.get_error();
...
int view::dummy_route_constructor::add_trajectory(map_view_object_h route_object,
					    maps_item_list_h points)
{
	...
	map_view_object_h polyline = pc.construct(NULL, points, 255, 0, 0, 0, 1);
	if(!polyline)
		return pc.get_error();
...
void view::dummy_route_constructor::add_start_marker(map_view_object_h route_object,
					       maps_route_h route)
{
...
void view::dummy_route_constructor::add_finish_marker(map_view_object_h route_object,
						maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
...
void view::dummy_route_constructor::add_route_path(map_view_object_h route_object,
					     maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
...
void view::dummy_route_constructor::add_route_segments(map_view_object_h route_object,
						 maps_route_h route)
{
	if(!route_object || !route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
...
map_view_object_h view::dummy_route_constructor::construct(maps_route_h route)
{
	if (!route) {
		__error = MAPS_ERROR_INVALID_PARAMETER;
		return NULL;
