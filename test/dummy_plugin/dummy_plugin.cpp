...
public:
	...
	map_view_object_h view_object;

	/* List of visual objects, comprising the route */
	map_view_object_h layer;

	/* Route Content */
	maps_route_h content;

public:
	maps_plugin_route(const map_view_object_h object,
		  ...
		  , content(route)
	{
		...
		layer = rc.construct(route);
	}

	~maps_plugin_route()
	{
		if(layer)
			_map_view_object_destroy(layer);
...
static void __remove_route(const map_view_object_h object)
{
	if(!object)
		return;
...
static void __store_route(const map_view_object_h object,
...
static maps_plugin_route *__get_route(const map_view_object_h object)
{
	if(!object)
		return NULL;
...
static void __draw_polyline(const map_view_object_h object,
			    unsigned int *pixels)
{
	...
	unsigned char b = 0;
	unsigned char a = 0;
	map_view_object_polyline_get_color(object, &r, &g, &b, &a);
...
static void __draw_polygon(const map_view_object_h object,
			   unsigned int *pixels)
{
	...
	unsigned char b = 0;
	unsigned char a = 0;
	map_view_object_polygon_get_fill_color(object, &r, &g, &b, &a);
	unsigned int color = ((1 | a) << 24)
		| ((1 | r) << 16)
		| ((1 | g) << 8)
		| b;
...
static void __draw_marker(const map_view_object_h object, unsigned int *pixels)
{
	...
	int error = map_view_object_marker_get_coordinates(object, &coords);
	if (error != MAPS_ERROR_NONE)
		return;
	...
	int screen_width = 0;
	int screen_height = 0;
	error = map_view_object_marker_get_screen_size(object,
						       &screen_width,
						       &screen_height);
	if (error != MAPS_ERROR_NONE)
		return;
	...
	map_view_object_marker_get_type(object, &type);
	switch(type) {
	case MAP_VIEW_MARKER_START:
	case MAP_VIEW_MARKER_FINISH: {
	...
	case MAP_VIEW_MARKER_FIRST:
	case MAP_VIEW_MARKER_SECOND:
	case MAP_VIEW_MARKER_FAVOURITE:
	case MAP_VIEW_MARKER_CONSTRUCTION:
	case MAP_VIEW_MARKER_NONE:
	default: {
		const int dx = screen_width / 2;
		const int dy = screen_height / 2;
...
static void __draw_layer(const map_view_object_h object, unsigned int *pixels);

static bool __for_each_map_view_object_cb(int index, int total,
					  map_view_object_h object,
					  void *user_data)
{
	/*g_print("__for_each_map_view_object_cb\n");*/

	if (!object)
		return false;

	bool visible = false;
	int error = map_view_object_get_visible(object, &visible);
	if (error != MAPS_ERROR_NONE)
		return false;
	...
	error = map_view_object_get_type(object, &type);
	if (error != MAPS_ERROR_NONE)
		return false;
	...
	case MAP_VIEW_OBJECT_ROUTE: {
		...
		__draw_layer(route->layer, pixels);
	...
	/*map_view_object_destroy(object);*/
	return true;
}

static void __draw_layer(const map_view_object_h object, unsigned int *pixels)
{
	/*g_print("__draw_layer\n");*/
	if (!object || !pixels)
		return;

	map_view_object_layer_foreach_object(object,
					     __for_each_map_view_object_cb,
					     pixels);
}
...
EXPORT_API int maps_plugin_on_view_object(const map_view_object_h object,
			       const map_view_object_operation_e operation)
{
	...
	case MAP_VIEW_OBJECT_ADD: {

		map_view_object_type_e type = MAP_VIEW_OBJECT_UNKNOWN;
		map_view_object_get_type(object, &type);
		if(type != MAP_VIEW_OBJECT_ROUTE)
			break; /* We are interested only in Route */

		break;
	}

	case MAP_VIEW_OBJECT_SET_VISIBLE:
		/* Ignore */
		break;

	case MAP_VIEW_OBJECT_MOVE:

		/* TODO: */

		break;

	case MAP_VIEW_OBJECT_CHANGE: {
		...
		int error = map_view_object_route_get_content(object,
							       &route_content);
		if(error != MAPS_ERROR_NONE)
			return error;
	...
	case MAP_VIEW_OBJECT_REMOVE:
		__remove_route(object);
		break;

	case MAP_VIEW_OBJECT_REMOVE_ALL:
		__remove_all_routes();
		break;
...
EXPORT_API int maps_plugin_get_min_tilt(int *min_tilt)
{
	if(!min_tilt)
		return MAPS_ERROR_INVALID_PARAMETER;
	*min_tilt = 0;
...
EXPORT_API int maps_plugin_get_max_tilt(int *max_tilt)
{
	if(!max_tilt)
		return MAPS_ERROR_INVALID_PARAMETER;
	*max_tilt = 60;
