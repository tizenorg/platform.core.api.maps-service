...
/*int map_event_data_get_scale_factor(map_event_data_h event,
					double *zoom_factor);*/
void utc_map_event_data_get_scale_factor_p(void)
{
	test_env e;
	...
	error = _map_event_data_set_scale_factor(e.e, 3.1);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);

	double zoom_factor = 0;
	error = map_event_data_get_scale_factor(e.e, &zoom_factor);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);
	g_assert_cmpfloat(zoom_factor, >=, 3);
}

void utc_map_event_data_get_scale_factor_n(void)
{
	test_env e;

	double zoom_factor = 0;
	int error = map_event_data_get_scale_factor(NULL, &zoom_factor);
	g_assert_cmpint(error, ==, MAPS_ERROR_INVALID_PARAMETER);

	error = map_event_data_get_scale_factor(e.e, NULL);
	g_assert_cmpint(error, ==, MAPS_ERROR_INVALID_PARAMETER);
}
