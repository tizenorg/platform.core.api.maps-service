...
void utc_map_event_data_get_tilt_p(void)
{
	test_env e;
	...
	error = _map_event_data_set_tilt(e.e, 3.1);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);

	double tilt = 0;
	error = map_event_data_get_tilt(e.e, &tilt);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);
	g_assert_cmpfloat(tilt, >=, 3);
}

void utc_map_event_data_get_tilt_n(void)
{
	test_env e;

	double tilt = 0;
	int error = map_event_data_get_tilt(NULL, &tilt);
	g_assert_cmpint(error, ==, MAPS_ERROR_INVALID_PARAMETER);

	error = map_event_data_get_tilt(e.e, NULL);
	g_assert_cmpint(error, ==, MAPS_ERROR_INVALID_PARAMETER);
}
