...
void utc_construct_marker_p(void)
{
	...
	map_view_object_h marker = mc.construct(NULL,
						maps::coordinates(10.1, 20.1),
						"test_marker.png",
						MAP_VIEW_MARKER_POI);
	...
	int error = map_view_object_marker_get_coordinates(marker, &coords);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);

	double lat = 0, lon = 0;
	error = maps_coordinates_get_latlon(coords, &lat, &lon);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);
	...
	char *file_path = NULL;
	error = map_view_object_marker_get_file_path(marker, &file_path);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);
	g_assert(file_path);
	...
	error = map_view_object_marker_get_type(marker, &type);
	g_assert_cmpint(error, ==, MAPS_ERROR_NONE);
